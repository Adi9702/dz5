#Инкапсуляция - объединение данных и методов, работающих с ними, в одном объекте, есть три метода 
...
#декораторы - это @ , он позволяет добавлять новое поведение к функции или классу без изменения их исходного кода.
#гетор - Это метод для получения значения приватного (или защищённого) атрибута.
#сеттеро - Это метод для изменения значения приватного атрибута. после сабачки идет имя так-как он должен знать, для какого свойства он работает


#Полиморфизм - есть одна команда которая действует всем классам (для этого используется родителький класс)
